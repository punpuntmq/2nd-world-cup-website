...
func BuildViewState(raw football.RawState, refreshInterval time.Duration, refreshMeta RefreshMeta) ViewState {
	now := time.Now().UTC()
	...
	status := fallback(refreshMeta.Status, refreshStatusIdle)
	...
	return ViewState{
		Meta: MetaView{
			...
			RemainingCalls:       refreshMeta.RemainingCalls,
			NextAllowedRefreshAt: formatTime(refreshMeta.NextAllowedRefreshAt),
			LastRefreshAt:        formatTime(refreshMeta.LastRefreshAt),
			IsStale:              refreshMeta.IsStale,
		},
		Teams:           teams,
...
func LiveMatchCandidate(raw football.RawState, now time.Time) (football.Match, bool) {
